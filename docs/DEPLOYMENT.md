...
3. `npm run validate`: 44 domain/lifecycle tests, TypeScript, production build, static artifact reference/hash verification, 20 production-build browser tests, whitespace check.
...
6. Run all 20 browser tests against the live endpoint. These interact only with local simulation state and download local JSON; no backend write endpoint exists.
...
The separately authorized custom domain is `dpl.aserdargun.com`. IHS holds the `dpl` CNAME pointing to the Azure-generated hostname and the `_dnsauth.dpl` TXT ownership record. No other DNS records are changed. No paid SKU, backend, external integration, model access, or other portfolio repository is added. The 2026-09-21 preflight confirmed one active production workflow; Azure reports GitHub provider metadata for this repository and `main`, with `allowConfigFileUpdates: true`. Preserve the existing deployment path and do not generate a second workflow. After DPL resource creation this subscription had 10 Free Static Web Apps.
