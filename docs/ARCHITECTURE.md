...
Vite bundles React + TypeScript to `dist/`. Navigation is local view state at `/`. Hosting requires only static assets; no backend, database, key, telemetry, or accounts. The subsequent authorized Azure deployment is described in `DEPLOYMENT.md`; the separately authorized `dpl.aserdargun.com` custom domain uses IHS DNS. Local lifecycle is loopback-only and strict-port; Stop inspects listener cwd before SIGTERM. Development port 8031, isolated production browser-test port 18032.
